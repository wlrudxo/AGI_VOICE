...
def _unique_object_name(raw_name: str, used_names: set[str], fallback: str) -> str:
    base = safe_name(raw_name or fallback) or fallback
...
def _append_traffic_vehicle(
    ...
    traffic_lines = [
        f"{prefix}.Name = {object_name or safe_name(vehicle.name)}",
        f"{prefix}.Info:",
...
def _append_pedestrian(
    ...
    lines.extend(
        [
            ...
            f"{prefix}.DetectMask = {PEDESTRIAN_DETECT_MASK}",
            f"{prefix}.UpdRate = 200",
