...
async function executeCreate(type: string, data: Record<string, any>): Promise<any> {
	if (type === 'map') {
		return await requestJson('/api/maps', { method: 'POST', body: data });
	} else {
...
async function readMap(data: Record<string, any>): Promise<string> {
	if (data.id) {
		// ID로 조회
		try {
			const map: any = await requestJson(`/api/maps/${data.id}`);
...
태그: ${map.tags || '없음'}`;
		...
		// 카테고리로 조회
		try {
			const maps: any[] = await requestJson(`/api/maps?category=${encodeURIComponent(data.category)}`);
			if (maps.length === 0) {
		...
		// 이름으로 검색
		try {
			const maps: any[] = await requestJson(`/api/maps?search=${encodeURIComponent(data.name)}`);
			if (maps.length === 0) {
...
async function executeUpdate(type: string, data: Record<string, any>): Promise<any> {
	if (type === 'map') {
		...
		return await requestJson(`/api/maps/${id}`, { method: 'PUT', body: updateData });
	} else {
...
async function executeDelete(type: string, data: Record<string, any>): Promise<any> {
	if (type === 'map') {
		return await requestJson(`/api/maps/${data.id}`, { method: 'DELETE' });
	} else {
