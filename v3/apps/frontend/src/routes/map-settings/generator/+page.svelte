...
<edges xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/edges_file.xsd">
	...
	async function loadMapData(id) {
		try {
			const map = await requestJson(`/api/maps/${parseInt(id)}`);
	...
	async function saveToDb() {
		...
		try {
			const map = await requestJson('/api/maps', {
				method: 'POST',
				body: {
					name: mapName.trim(),
	...
	async function updateMap() {
		...
		try {
			const map = await requestJson(`/api/maps/${editMapId}`, {
				method: 'PUT',
				body: {
					name: mapName.trim(),
					description: mapDescription.trim(),
	...
	async function deleteMap() {
		...
		try {
			await requestJson(`/api/maps/${editMapId}`, { method: 'DELETE' });
	...
	// Import XML files via standard file input
	...
	async function handleFileInput(event) {
		...
		try {
			...
			const edgFiles = files.filter(f => f.name.endsWith('.edg.xml'));
			const nodFiles = files.filter(f => f.name.endsWith('.nod.xml'));

			// Validation: Only 1 of each type allowed
			if (edgFiles.length > 1) {
				saveMessage = { type: 'error', text: '.edg.xml 파일은 1개만 선택할 수 있습니다.' };
				setTimeout(() => { saveMessage = null; }, 3000);
				event.target.value = '';
				return;
			}
			...
			if (nodFiles.length > 1) {
				saveMessage = { type: 'error', text: '.nod.xml 파일은 1개만 선택할 수 있습니다.' };
				setTimeout(() => { saveMessage = null; }, 3000);
				event.target.value = '';
				return;
			}
			...
			if (nodFiles.length === 1) {
				const content = await nodFiles[0].text();
				nodeXml = content;
				console.log('Loaded node XML from:', nodFiles[0].name);
			}

			if (edgFiles.length === 1) {
				const content = await edgFiles[0].text();
				edgeXml = content;
				console.log('Loaded edge XML from:', edgFiles[0].name);
			}
...
<input type="file" accept=".xml" multiple bind:this={fileInputEl} onchange={handleFileInput} style="display:none" />
