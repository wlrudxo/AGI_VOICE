<script>
	...
	async function loadMaps() {
		try {
			...
			const response = await requestJson('/api/maps');
	...
	async function handleDeleteMap(map) {
		...
		try {
			await requestJson(`/api/maps/${map.id}`, { method: 'DELETE' });
	...
	async function handleEmbedMap(map) {
		...
		try {
			...
			const result = await requestJson(`/api/maps/${map.id}/embed`, { method: 'POST' });
	...
	async function createSampleMaps() {
		...
		let existingMaps = [];
		try {
			existingMaps = await requestJson('/api/maps');
		} catch (e) {
...
<edges xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://sumo.dlr.de/xsd/edges_file.xsd">
	...
	async function handleBuildAllEmbeddings() {
		...
		try {
			...
			const result = await requestJson('/api/maps/embeddings/build-all', { method: 'POST' });
