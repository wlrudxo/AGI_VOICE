<script lang="ts">
  ...
  async function sendControl(controlType: string, value: number) {
    try {
      ...
      if (controlType === 'Gas') {
        result = await requestJson('/api/carmaker/control/gas', { method: 'POST', body: { value, duration } });
      } else if (controlType === 'Brake') {
        result = await requestJson('/api/carmaker/control/brake', { method: 'POST', body: { value, duration } });
      } else if (controlType === 'Steer.Ang') {
        result = await requestJson('/api/carmaker/control/steer', { method: 'POST', body: { value, duration } });
      } else if (controlType === 'LaneOffset') {
        // Send DM.LaneOffset command directly
        const command = `DVAWrite DM.LaneOffset ${value} ${duration} Abs`;
        result = await requestJson('/api/carmaker/command', { method: 'POST', body: { command } });
  ...
  async function executeCommand() {
    ...
    try {
      const result = await requestJson('/api/carmaker/command', { method: 'POST', body: { command: commandInput } });
